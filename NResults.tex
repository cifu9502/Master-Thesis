...
\section{NRG for the interacting system}
...
\caption{ \label{fig:Nt1=t2} The same as in \ref{fig:t1=t2} for the  interacting DOS in the symmetric coupling setup (\ref{fig:MajoranaModels}.
}
%
\end{center}
\end{figure}
%-----------E N D  F I G U R E  7 ------
    ...
    In this part project we are interested in the physics at low energy scales $\omega \sim \Gamma_1 $ close to the Kondo and MZM temperature. At this scale we can observe similar Majorana signatures compared with the non-interacting case. For instance, \ref{fig:Nt1=t2} shows the NRG results for the symmetric setup in \ref{fig:MajoranaModels}(a). In agreement with the non-interacting results, both dots have type I Majorana signatures. These signatures can be manipulated by tuning one of the dot's gate voltage to induce the MZM to leak into the other dot. However, the DOS at figures \ref{fig:Nt1=t2}(d)(e) are not so clear as in the non-interacting case.   

\begin{figure}[H]
    \centering
    \includegraphics[scale=0.6]{IMAGES/NRG/Msig.png}
    \caption{\label{fig:ed2/Fermi}  This picture  shows the evolution of the relation $\frac{\rho_\up(0)}{\rho_\up(0)}$ for both QDs. While QD2 losses rapidly the Majorana signature, QD1 maintains it till $\Delta \ed{2}\sim 5$.}
\end{figure}


    Still it seems that the second dot has a type I Majorana signature with $\rho_\dw(0) \approx \frac{1}{2}\rho_\up(0))$.
    We can verify this in \ref{fig:ed2/Fermi} which shows the evolution of the DOS for an increasing second dot's gate voltage.  The Majorana signature is stable at $\frac{rho_\up(0)}{rho_\dw(0)}\approx 2$  for displacements of the gate votage bellow  $5\Gamma_1$. At larger displacements the signal in the first dot is totally destroy destroyed. On the other hand, the second dots gate voltage is smoothly suppressed. 
...
\caption{  \label{fig:Nt1>0} The same as in \ref{fig:t1=t2} for the  interacting DOS of the setup in \ref{fig:MajoranaModels}(b). Inset in b): Low-energy DOS. 
}
%
\end{center}
\end{figure}
...
%-----------E N D  F I G U R E  4 ------
    ...
    Finally, \ref{fig:Nt2>0} show the NRG results for the last configuration \ref{fig:MajoranaModels}(c). Surprisingly, the indirectly-attached MZM exhibits a robust type II Majorana signature in the first dot over a destroyed Kondo peak. This signature is stable under the gate voltage tuning. In addition, only  in the particle hole symmetric case the second dot presents a type II Majorana signature (Inset \ref{fig:Nt2>0}(b)). We could understand this effect by thinking that the QDs in model (c) are attached in series. Therefore the two dots can be thought as extensions of the Kitaev chain being the first dot  the last place in the wire. Hence the Majorana should be localized at this dot despite the application of gate voltages. This case is similar to the case of a single dot attached to a Majorana chain, where it is known that the MZM appears in the dot even when this is supposed to be empty \cite{vernek_subtle_2014}. It still remains the doubt about why this effect is not observed in the non-interacting case . On the other hand, there is a significant zero-mode in the spin-$\dw$ DOS. This mode was not identified as a potential Majorana signature since it increases when $\delta \ep_2$. 
    \begin{figure}[bt]
    \begin{center}
    \includegraphics[scale=0.45]{IMAGES/NRG/t2>0.png}
    \caption{  \label{fig:Nt2>0} The same as in \ref{fig:t1=t2} for the  interacting DOS of the setup in \ref{fig:MajoranaModels}(b). Inset in b): Low-energy DOS. 
    }
    %
    \end{center}
    \end{figure}
    ...
    All these manipulations are summarized in \ref{fig:ResultModels}
    \begin{figure}
        \begin{center}{}
        \includegraphics[scale=0.45]{IMAGES/Graphs/Majorana_Old_Models.png}
