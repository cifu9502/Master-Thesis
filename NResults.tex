...
% ------------------------FIGURE GRAPH--------------------
     \begin{figure}[bt]
    \centering
    \includegraphics[scale=0.4]{IMAGES/Graphs/Final Graph.png}
...
This simple formula summarizes the transport information through the first dot of the non-interacting Majorana-DQD system.  To compute the DOS we just need to replace once again  $\sum \frac{V_1V^*_1}{\omega -\epsilon_k}= -i\Gamma_1$ as performed in \ref{sec:GraphMethod}. By plotting the final DOS in Mathematica we were able to observe the transitions of the Majorana mode while manipulating the model parameters.  We obtained three models that are particularly interesting for the exotic behavior of their Majorana signature. 
