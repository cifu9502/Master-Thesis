...
\begin{dedication}
\textit{ To a little cheek }
\end{dedication}

\chapter*{Acknowledgments}
We thank Edson Vernek for enlightening discussions. We also acknowledges the financial support by CNPq (grants No. 307107/2013-2 and 449148/2014-9), and FAPESP (grant No. 2016/18495-4), which allowed us to present partial and complete results of this project in workshops and conferences. 

\begin{abstract}

Majorana zero modes (MZMs) emerging at the edges of topological superconducting wires are a promising platform for fault-tolerant quantum computation. Novel proposals quantum dots (QDs) to detect Majorana signatures. This detection method provides the following advantages: 1) QDs are the best device to study the coexistence of Kondo-Majorana signatures, which have been recently reported in experiments.  2)  Today's precise experimental control over QDs offers the unique possibility of manipulating the Majoranas inside multi-dot systems, which is turning new lights into the design of quantum architectures.  

% These two features motivated our study Kondo-Majorana signatures in double quantum dots (DQD). 

The simplest case where Majorana manipulation is possible is in a double quantum dot (DQD). This model shows several possibilities for manipulation of MZMs, including different geometric configurations of the dots from symmetric and  linear couplings to T-dot junctions. In this project we perform a theoretical study  of the transitions of the Majorana signature in a DQD in the non-interacting and interacting regimes. By tuning the dot's gate voltages, we will show that it is possible to control the localization of the MZM inside the DQD.  We will explore the interplay of these signatures with the Kondo effect. 

We adopt two methods in this project: 1) The green equations of motion (EOM) allow us to obtain exact expressions for the density of states in coulomb-non-interacting systems. We present the Graph -Gauss-Jordan elimination process as a simple-graphical method to solve the emergent linear systems in the EOM. 2) We use the numerical renormalization group (NRG) in interacting systems, where the Kondo effect emerges allowing us to observe combined Kondo-Majorana physics. We will test these methods first in a double quantum dot (DQD) (\ref{chap: Methods}) and then in a QD-Majorana model (\ref{sec:QD-Majorana})  confirming the results of previous papers  \cite{dias_da_silva_transmission_2008,liu_detecting_2011,ruiz-tijerina_interaction_2015}.  Finally, we include the main contribution of this thesis, the study of a DQD coupled to a Majorana chain (\ref{chap:Results}). 
...
As quasi-particulas de Majorana que emergen nas bordas de un superconductor topologico s\~ao uma plataforma prometedora na computa\c{c}\~ao quantica. Novas propostas usam quantum dots (QDs) para detectar signaturas de Majorana. Este m\'etodo tem duas vantagens: 1) Os QDs  s\~ao o melhor dispositivo para estudar a co-exist\^encia de signaturas de Kondo e Majorana, as quais t\^em sido reportadas recentemente em experimentos. 2) O control experimental preciso sobre os quantum dots que temos hoje em dia oferece a oportunidade \'unica para manipular quasi-part\'iculas de Majoranas dentro de sistemas com v\'arios dots. Esta ideia abriu novos caminhos para o desenho de arquiteturas qu\^anticas. 


O caso mais simples em que \'e poss\'ivel manipular tais quasi-part\'iculas \'e num dobro quantum dot (DQD). Este modelo oferece v\'arias possibilidades para mover os Majorana, incluindo m\'ultiplas configura\c{c}\~oes geom\'etricas dos dots como acoples sim\'etricos, lineares e em junturas T. Neste trabalho vamos apresentar uma an\'alise te\'orico das transiciones das signaturas de Majorana dentro do DQD em sistemas interatuantes e n\~ao interatuantes. Vamos ver que \'e poss\'ivel controlar a localiza\c{c}\~ao do modo zero de Majorana  mediante o incremento nos voltagens de gate dos QD. Tamb\'em vamos explorar como essas signaturas interagem com o efeito Kondo.

Principalmente, vamos a usar dois m\'etodos neste projecto: 1) Usamos as equa\c{c}\~oes de movimento no formalismo the fun\c{c}\~oes de green para  obter express\~oes exatas para a densidade de estados em sistemas n\~ao interatuantes.  Vamos apresentar o m\'etodo the elimina\c{c}\~ao de Gauss-Jordan com grafos, o qual permite resolver rapidamente o sistema linear emergente nas equa\c{c}\~oes de movimento. 2) Em sistemas Coulomb interatuantes usamos NRG, no qual vamos poder observar a intera\c{c}\~ao entre o Majorana e o efeito Kondo. Vamos testar ambos m\'etodos nos modelos de um double quantum dot e um QD acoplado com uma cadeia de Majorana, com o qual vamos reproduzir os resultados de paper anteriores. Finalmente, inclu\'imos a maior contribui\c{c}\~ao deste trabalho, o estudo de um DQD acoplado a uma cadeia de Majorana. 
