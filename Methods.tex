...
The method presented here uses graph theory algorithms that provide a shortcut to Gauss-Jordan elimination \cite{s}.

These graphs allow us to reduce the complexity of the process by understanding the 
...
\end{align}
 ...
 Note that we only wrote  the green functions having $d_1^\dagger$ as second operator. This system is already closed which means that we don't need any other equation to find the solution. The matrix equation corresponding to this linear equation is 
 \begin{equation}
...
\end{array}\right]
 ...
 To simplify future computations we changed the order of the rows in the matrix.  Now 
