...
\subsubsection{Iterative Diagonalization in a Single QD Hamiltonian}
...
\begin{equation}
  \vert N_{1},S_{1}\rangle\otimes\vert N_{2},S_{2}\rangle\subset\vert N_{1}+N_{2},S_{1}+S_{2}\rangle \label{eq:PropRuleQD}
\end{equation} 
...
Finally, the spectrum for $\Lambda = 2.5$ takes the 'spaghetti' form in \ref{Fig-Dot-Spectrum}. Before $N=30$ low-energy contributions generate significant changes in the energy levels.  The stable strum after the step $N=30$ shows that the code has converged at this stage. As we previously declared, it is not $T$ but $T^2$ the transformation that has fixed points, which explains why it was necessary to plot the even and the odd spectrum separately.
...
\subsection{Details of the Code}


The NRG code implemented in C++ by my advisor Luis Gregorio Dias takes in account all of the details described in this chapter. 
